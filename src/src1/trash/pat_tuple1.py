...
#This is done in order to generate cfg files automatically
DATALIST=[
...
# There are some differences between something called "fastsim" and
# regular simulation, as well as between different versions of the MC
# event generator Pythia. See their use below.

isFastsim = False
isFastsimWithBug = False
isPythia8 = False
...
"file:/afs/cern.ch/work/j/jgomezca/thesis/CMSSW_6_0_0/src/bTagAnalyzer/BTAGPerformance/files/TT.root"
    )
                            )
...
process.out = cms.OutputModule('PoolOutputModule',
                               ...
                               SelectEvents = cms.untracked.PSet(SelectEvents = cms.vstring('pSemileptonic')),
...
# Depending on the particular version of CMSSW used to generate the
# fastsim sample you use, these three lines may be necessary to patch
# a bug, but it can also lead to crashes if a newer CMSSW (where the
# bug was fixed) was used.
...
# Pythia version 8 uses different (numerous) status codes for its
# particles than version 6. In the PAT tools above, the MC matching by
# default looks for particles that have status 3 -- this is replaced
# by the codes 21-29 in Pythia 8. (We don't need to check runOnMC here
# -- this info just won't be used if runOnMC is False.
#
# JMTBAD should also see what MC matching is doing for e.g. muons in
# fastsim.
...
process.patJetPartonMatch1 = process.patJetPartonMatch.clone(src = cms.InputTag("selectedPatJetsPF"),
                                                             mcStatus=[1]
                                                             )
...
if __name__ == '__main__' and 'submit' in sys.argv:
    ...
    for dataset_ in DATALIST:
        ...
        pset = pat_tuple1_crab.py
        output_file = pat.root
        %(jobcontrol_)s
        
        [USER]
        ui_working_dir = /afs/cern.ch/work/j/jgomezca/public/ForUniandes/CMSSW_5_3_9/resultsCrab/pattuples/%(dirname_)s
        return_data = 1
        copy_data               = 0
        storage_element         = T2_CH_CERN
        user_remote_dir         = JPThesis1/
        publish_data=0
        eMail                   = djp@cern.ch
        ''' % locals()
        open('crab.cfg', 'wt').write(crab_cfg)
        open('pat_tuple1_crab.py', 'wt').write(new_py)
        if 'test' not in sys.argv:
            os.system('crab -create -submit all')
            os.system('rm crab.cfg')
            os.system('rm pat_tuple1_crab.py')
