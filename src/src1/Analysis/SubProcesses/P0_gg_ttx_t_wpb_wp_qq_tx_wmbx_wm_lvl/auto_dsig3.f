...
C     ----------
      ...
      PD(0) = 0D0
      IPROC = 0
      IPROC=IPROC+1  ! d d~ > u d~ b e- ve~ b~
      PD(IPROC)=PD(IPROC-1) + D1*DX2
      IPROC=IPROC+1  ! s s~ > u d~ b e- ve~ b~
      PD(IPROC)=PD(IPROC-1) + S1*SX2
      IPROC=IPROC+1  ! d d~ > c s~ b e- ve~ b~
      PD(IPROC)=PD(IPROC-1) + D1*DX2
      IPROC=IPROC+1  ! s s~ > c s~ b e- ve~ b~
      PD(IPROC)=PD(IPROC-1) + S1*SX2
      IPROC=IPROC+1  ! d d~ > u d~ b mu- vm~ b~
      PD(IPROC)=PD(IPROC-1) + D1*DX2
      IPROC=IPROC+1  ! s s~ > u d~ b mu- vm~ b~
      PD(IPROC)=PD(IPROC-1) + S1*SX2
      IPROC=IPROC+1  ! d d~ > c s~ b mu- vm~ b~
      PD(IPROC)=PD(IPROC-1) + D1*DX2
      IPROC=IPROC+1  ! s s~ > c s~ b mu- vm~ b~
      PD(IPROC)=PD(IPROC-1) + S1*SX2
      CALL SMATRIX3(PP,DSIGUU)
      IF (DSIGUU.LT.1D199) THEN
        DSIG3=PD(IPROC)*CONV*DSIGUU
      ELSE
